...
template<typename T> lean_object* mkLoosePtr(void* ptr) {
...
template<typename T> lean_object* mkOwnedPtr(T* ptr) {
	return lean_alloc_external(getOwnedPtrClass<T>(), ptr);
}

// Get the Lean-owned pointer wrapped in an object.
template<typename T> T* fromOwnedPtr(lean_object* obj) {
...
static inline lean_object* borrowLink(lean_object* linkedPtrObj) {
	return lean_ctor_get(linkedPtrObj, 0);
}

// Get the object linked to the a linked pointer object and increment its RC.
static inline lean_object* shareLink(lean_object* linkedPtrObj) {
...
// Get the Lean-owned pointer wrapped in an object.
template<typename T> T* fromLinkedOwnedPtr(lean_object* obj) {
	return fromOwnedPtr<T>(lean_ctor_get(obj, 1));
}

// Wrap a loose pointer in a Lean LinkedPtr.
template<typename T> lean_object* mkLinkedLoosePtr(b_lean_obj_arg link, T* ptr) {
	lean_object* obj = lean_alloc_ctor(0, 2, 0);
	lean_ctor_set(obj, 0, link);
...
template<typename T> T* fromLinkedLoosePtr(lean_object* obj) {
